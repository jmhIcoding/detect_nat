...
struct _packet_statics_feature
{
	...
	void _vectorize(char * dst,int bufSize=512,int ipflag=0)
		//最后一个ipflag用于表示是否需要把ip 写入特征字符串中
		//向量的格式：
		/*
		label 特征0 特征1 特征2 ....\n
		label 特征0 特征1 特征2 ....\n
		...
		*/
	{
		...
		sprintf(
			dst,
			"%s %d %d %d %d %d %d %d %d %d %d %d %d %d %f %f %f %f %d %d %f;", 
			inet_ntoa(addr),
			device_type,
		...
		else if(ipflag==0)
		{
			sprintf(
			dst,
			"%d %d %d %d %d %d %d %d %d %d %d %d %d %f %f %f %f %d %d %f;", 
			device_type,
			numInPkt, numInByte, numOutPkt, numOutByte,
			numHTTP, numTCP, numUDP, numDNSReq,
			numSYN, numRST, numFIN,
